...
namespace WeNotWork
{
    ...
    public class Startup
    {
        ...
        public static bool RunOnStartup(string AppTitle, string AppPath)
        {
            ...
            try
            {
                rk = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true) ?? throw new ArgumentNullException("no reg key");
            ...
            try
            {
                rk = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true) ?? throw new ArgumentNullException("no reg key");
                rk.SetValue(AppTitle, AppPath);
            }
            catch (Exception)
            {
